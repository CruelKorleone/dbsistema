...
public class IngresoDAO implements CrudIngresoInterface<Ingreso, DetalleIngreso> {
    ...
    @Override
    public List<Ingreso> listar(String texto, int totalPorPagina, int numPagina) throws Exception {
        List<Ingreso> registros = new ArrayList();
        try {
            ps = CON.conectar().prepareStatement("SELECT i.id, i.usuario_id, u.nombre as usuario_nombre, i.persona_id, "
                    + "p.nombre as persona_nombre, i.tipo_comprobante, i.tim_comprobante,i.num_comprobante, i.fecha, i.impuesto,i.total, "
    ...
    @Override
    public boolean insertar(Ingreso obj) throws ClassNotFoundException {
        resp = false;
        Connection conn = null;
        try {
            conn = CON.conectar();
            conn.setAutoCommit(false);
            String sqlInsertIngreso = "INSERT INTO ingreso (persona_id, usuario_id, fecha, tipo_comprobante, tim_comprobante, num_comprobante, impuesto, total, estado) VALUES (?,?,now(),?,?,?,?,?,?)";
            ...
            ps.setString(4, obj.getTimComprobante());
            ...
            try {
                ...
                java.util.logging.Logger.getLogger(IngresoDAO.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
            }
        } finally {
            try {
                if (rs != null) {
    ...
    public boolean existe(String texto1,String texto2) throws ClassNotFoundException {
       resp = false;
        try {
            ps = CON.conectar().prepareStatement("SELECT id FROM ingreso WHERE tim_comprobante=? AND num_comprobante=?");
